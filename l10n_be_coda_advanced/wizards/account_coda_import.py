...
class AccountCodaImport(models.TransientModel):
    ...
    def _st_line_reconcile_thread(self, wiz_dict, st_lines, reconcile_note):
        ...
        for st_line in st_lines:
            ...
            try:
                ...
                log_err = f"{exctype.__name__}: {str(value)}"
    ...
    def _get_sale_order(self, wiz_dict, st_line, cba, transaction, reconcile_note):
        ...
        select = (
            f"SELECT id FROM (SELECT id, name, '{free_comm}'::text AS free_comm, "
            "regexp_replace(name, '[0]{{3,10}}', '0%%0') AS name_match "
            "FROM sale_order WHERE state not in ('cancel', 'done') "
            f"AND company_id = {cba.company_id.id}) sq "
        ...
        select = f"""
            SELECT id FROM account_move
              WHERE payment_state != 'paid'
                AND company_id = {cba.company_id.id}
                AND state = 'posted'
                AND round(amount_total, 2) = {amount_rounded}
        """
    ...
    SELECT id FROM (
      ...
      '{free_comm_digits}'::text AS free_comm_digits
        FROM account_move
