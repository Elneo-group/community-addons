...
class AccountCodaImport(models.TransientModel):
    ...
    def _coda_record_1(self, coda_statement, line, coda_parsing_note):  # noqa: C901
        ...
        if cba:
            if cba.company_id not in self.env.companies:
                ...
                ) % (coda_statement["acc_number"], cba.company_id.name)
                skip = True
            else:
                # rebrowse to enforce record rules
                cba = self.env["coda.bank.account"].browse(cba.id)
            coda_statement["coda_bank_params"] = cba
            self._company_bank_accounts = cba.company_id.bank_journal_ids.mapped(
                "bank_account_id"
    ...
    def _coda_record_2(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_21(  # noqa: C901
        self, coda_statement, line, coda_parsing_note, transaction_seq
    ):
        ...
        trans_type = [
            x for x in self._trans_types if transaction["trans_type"] == x.type
        ]
        if not trans_type:
            err_string = (
        ...
        trans_category = [
            x for x in self._trans_categs if transaction["trans_category"] == x.category
        ]
        if trans_category:
        ...
        if line[61] == "1":
            ...
            comm_type = [
                x for x in self._comm_types if x.code == transaction["struct_comm_type"]
            ]
            if not comm_type:
                err_string = (
                    _(
    ...
    def _coda_record_22(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_23(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_3(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_31(self, coda_statement, line, coda_parsing_note, transaction_seq):
        ...
        trans_type = [x for x in self._trans_types if x.type == info_line["trans_type"]]
        if not trans_type:
            err_string = (
                _("\nThe File contains an invalid CODA Transaction Type : %s !")
        ...
        trans_category = [
            x for x in self._trans_categs if x.category == info_line["trans_category"]
        ]
        if trans_category:
        ...
        if line[39] == "1":
            ...
            comm_type = [
                x for x in self._comm_types if x.code == info_line["struct_comm_type"]
            ]
            if not comm_type:
                err_string = (
                    _(
            ...
            info_line["communication"] = INDENT8 + name + ":"
            info_line["communication"] += INDENT8 + info
    ...
    def _coda_record_32(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_33(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_4(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _coda_record_8(self, coda_statement, line, coda_parsing_note, transaction_seq):
    ...
    def _prepare_statement_line(  # noqa: C901
        self, coda_statement, transaction, coda_parsing_note
    ):
        ...
        if transaction["type"] in ["globalisation", "regular"]:
            ...
            self._format_transaction_note(coda_statement, transaction)
    ...
    def _coda_parsing(self, codafile, codafilename):  # noqa: C901
        ...
        self._trans_categs = self.env["account.coda.trans.category"].search([])
        ...
        for coda_statement in coda_statements:
            ...
            for transaction in bank_st_transactions:
                ...
                self._create_bank_statement_line(coda_statement, transaction)
            ...
            self._coda_import_note = self._coda_import_note + _(
                "\n\nBank Journal: %s"
                "\nCODA Version: %s"
                ...
                coda_statement["coda_parsing_note"] % {"name": coda_statement["name"]},
            )
    ...
    def _automatic_reconcile(self, statement, reconcile_note="", st_lines=None):
        """
        ...
        lines = statement.line_ids.filtered(
            lambda r: r.amount and r.coda_transaction_dict and not r.moves_state
        )
        if st_lines:
    ...
    def _st_line_reconcile_thread(self, st_line_ids, reconcile_note):
        ...
        with api.Environment.manage():
            with registry(self.env.cr.dbname).cursor() as cr:
                ...
                st_lines = env["account.bank.statement.line"].browse(st_line_ids)
                ...
                for st_line in st_lines:
                    ...
                    try:
                        reconcile_note = env["account.coda.import"]._st_line_reconcile(
    ...
    def _st_line_reconcile(self, st_line, cba, transaction, reconcile_note):
        ...
        if not match.get("counterpart_amls") or match.get("account_id"):
        ...
        if match.get("partner_id"):
            st_line.write({"partner_id": match["partner_id"]})
    ...
    def _match_and_reconcile(self, st_line, cba, transaction, reconcile_note):
        ...
        # match on invoice
        reconcile_note = self._match_invoice(st_line, cba, transaction, reconcile_note)
        if match_status in ["break", "done"]:
            return reconcile_note
    ...
    def _match_payment_reference(self, st_line, cba, transaction, reconcile_note):
        """
    ...
    def _match_invoice_number(
        self, st_Line, cba, transaction, reconcile_note, free_comm
    ):
        """
    ...
    SELECT id FROM (
      ...
      WHERE round(amount_total, 2) = %s
        ...
        if transaction["amount"] > 0:
            select2 = " AND type = 'out_invoice' AND free_comm ilike '%'||name||'%'"
            ...
            if res:
                ...
                select2 = (
                    " AND type = 'in_refund' AND"
                    " free_comm ilike '%'||invoice_payment_ref||'%'"
                )
                self.env.cr.execute(select + select2)  # pylint: disable=E8103
                res = self.env.cr.fetchall()
                if res:
        ...
        # 'in_invoice', 'out_refund'
            ...
            if res:
                ...
                select2 = " AND type = 'out_refund' AND free_comm ilike '%'||name||'%'"
                self.env.cr.execute(select + select2)  # pylint: disable=E8103
                res = self.env.cr.fetchall()
                if res:
                    inv_ids = [x[0] for x in res]
    ...
    def _match_invoice(self, st_line, cba, transaction, reconcile_note):  # noqa: C901
        ...
        if match.get("invoice_id"):
            match["status"] = "done"
            invoice = self.env["account.move"].browse(match["invoice_id"])
    ...
    def _match_account_move_line(self, st_line, cba, transaction, reconcile_note):
        """
    ...
    def _match_counterparty(self, st_line, cba, transaction, reconcile_note):
        ...
        transfer_accounts = [x for x in self._company_bank_accounts if cp_number in x]
        if transfer_accounts:
            ...
            if cp_number not in get_iban_and_bban(cba.bank_id.sanitized_acc_number):
                match["status"] = "done"
                match["account_id"] = cba.transfer_account_id.id
    ...
    def _unlink_duplicate_partner_banks(
        self, st_line, cba, transaction, reconcile_note, partner_banks
    ):
        """
    ...
    def _update_partner_bank(self, st_line, cba, transaction, reconcile_note):
        ...
        if (
            match.get("partner_id")
            and cp
            and match.get("account_id") != cba.transfer_account_id.id
        ):
    ...
    def _format_line_notes(self, st_line, cba, transaction, line_notes, force=False):
        if not self.reconcile_matching_details and not force:
            return ""
        note = INDENT4
        note += _("Bank Statement '%s' line '%s':") % (
    ...
    def _parse_comm_move(self, coda_statement, transaction):
        ...
        if method_name in dir(self):
            ...
            st_line_name, st_line_comm = method_instance(coda_statement, transaction)
    ...
    def _parse_comm_move_108(self, coda_statement, transaction):
        ...
        st_line_name = _("Closing, period from %s to %s") % (period_from, period_to)
        comm_fields = [
            {
                "name": "amount_currency_account",
                "label": _("Equivalent in the currency of the account"),
    ...
    def _parse_comm_info_001(self, coda_statement, transaction):
        ...
        st_line_comm = INDENT8 + st_line_name + ":"
        val = comm[0:70].strip()
        if val:
            st_line_comm += INDENT8 + _("Name") + ": %s" % val
        val = comm[70:105].strip()
        if val:
            st_line_comm += INDENT8 + _("Street") + ": %s" % val
        val = comm[105:140].strip()
        if val:
            st_line_comm += INDENT8 + _("Locality") + ": %s" % val
        val = comm[140:175].strip()
        if val:
            st_line_comm += INDENT8 + _("Identification Code") + ": %s" % val
        return st_line_name, st_line_comm
    ...
    def _parse_comm_info_006(self, coda_statement, transaction):
        ...
        st_line_comm = INDENT8 + st_line_name + ":"
        st_line_comm += (
            INDENT8 + _("Description of the detail") + ": %s" % comm[0:30].strip()
        )
        st_line_comm += INDENT8 + _("Amount") + ": %s" % amount
        st_line_comm += INDENT8 + _("Category") + ": %s" % comm[49:52].strip()
        return st_line_name, st_line_comm
    ...
    def _parse_comm_info_007(self, coda_statement, transaction):
        ...
        st_line_comm = INDENT8 + st_line_name + ":"
        st_line_comm += INDENT8 + _("Number of notes/coins") + ": %s" % comm[0:7]
        st_line_comm += INDENT8 + _("Note/coin denomination") + ": %s" % comm[7:13]
        st_line_comm += INDENT8 + _("Total amount") + ": %.2f" % list2float(comm[13:28])
        return st_line_name, st_line_comm
    ...
    def _parse_comm_info_008(self, coda_statement, transaction):
        ...
        st_line_comm = INDENT8 + st_line_name + ":"
        st_line_comm += INDENT8 + _("Name") + ": %s" % comm[0:70].strip()
        st_line_comm += (
            INDENT8 + _("Identification Code") + ": %s" % comm[70:105].strip()
        )
        return st_line_name, st_line_comm
