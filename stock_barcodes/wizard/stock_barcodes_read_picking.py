...
class WizStockBarcodesReadPicking(models.TransientModel):
    ...
    def process_barcode(self, barcode):
        res = super(WizStockBarcodesReadPicking, self).process_barcode(barcode)
        picking = self.env["stock.picking"].search([('name','=',barcode)])
        if picking:
            self.picking_id = picking
            self._set_messagge_info("info", _("Picking found"))
            return
        return res
