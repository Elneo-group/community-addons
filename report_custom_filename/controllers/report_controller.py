...
class ReportController(ReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        for report in report_xml.browse(report_ids):
            ...
            # UPDATE LOIC : translate filename
            translation_id = http.request.env['ir.translation'].search([('name','=','ir.actions.report.xml,download_filename'),('src','=',report.download_filename),('lang','=',report.env.user.lang)])
            download_filename = translation_id.value if translation_id else report.download_filename 
            
            generated_filename = email_template.mako_template_env\
                .from_string(download_filename)\
                .render({
                    'objects': objects,
                    'o': objects[:1],
                    'object': objects[:1],
