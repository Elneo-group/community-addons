...
class AccountCodaImport(models.TransientModel):
    ...
    def _match_payment_line(self, st_line, cba, transaction, payline, reconcile_note):
        """
        Remark:
        ...
        if aml.reconciled:
            ...
            if transfer_account and amt_fld:
                ...
                cp_aml = rec_amls - aml
                transfer_aml = cp_aml.move_id.line_ids.filtered(aml_filter)
                match["counterpart_amls"] = [(transfer_aml, amt_paid)]
        elif amt_fld and cur.is_zero(amt_paid - getattr(aml, amt_fld)):
            match["counterpart_amls"] = [(aml, amt_paid)]

        return reconcile_note

    def _skip_payment_reference_match(self, st_line, cba, transaction, reconcile_note):
        skip = False
