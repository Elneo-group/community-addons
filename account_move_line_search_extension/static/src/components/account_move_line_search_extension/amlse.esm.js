...
export class accountMoveLineSearchExtensionListRenderer extends ListRenderer {
        ...
        onWillStart(async () => {
            if (this.is_custom_buttons_already_rendered !== true) {
                const result = await this.orm.call(
                    ...
                    "search_read",
                    [],
                    {fields: ["name"], context: self.userService.context}
                );
    ...
    _apply_custom_search_domain(domain) {
        var special_char = "🌟";
        var dico = this.env.searchModel.facets;
        for (var i in dico) {
            if (dico[i].values.includes(special_char)) {
                this.env.searchModel.deactivateGroup(dico[parseInt(i, 10)].groupId);
            }
        }
